...
get_latest_release() {
  fetch "https://api.github.com/repos/hostmyapps-git/markdown-preview.nvim/releases/latest" |
    grep '"tag_name":' |
    sed -E 's/.*"([^"]+)".*/\1/'
}
...
download() {
  mkdir -p bin
  cd bin
  url="https://github.com/hostmyapps-git/markdown-preview.nvim/releases/download/$tag/${1}"
...
arch=$(uname -sm)
case "${arch}" in
  "Linux x86_64") download markdown-preview-linux-x64.tar.gz ;;
  "Linux arm64") download markdown-preview-linux-arm64.tar.gz ;;
